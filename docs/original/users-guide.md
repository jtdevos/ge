# Galactic Empire — Users Guide v3.2 (transcription)

Faithful transcription of `mbmgemp/MBMGEMP.DOC` (the newest of the
three identical-but-for-the-address copies in the repository — this
one carries the author's later St. Cloud address and email). Only the
formatting is converted: page breaks and page padding removed, section
titles promoted to markdown headings, terminal-layout blocks (menus,
maps, syntax displays) fenced. Every word, line break and typo of the
original text is preserved — including the misnumbered section
"11.145 JETTESON", the duplicated "11.35", and spellings like "rouge
ships". Do not "fix" this file; corrections belong in the
reconstructed manual (`docs/manual/`), not the artifact.

Transcription verified against the source with
`salvage/tools/check_transcription.py`.
